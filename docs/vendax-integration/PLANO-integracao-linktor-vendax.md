...
| **L1 · Channel config** | Consumir `core.channel.config` → materializar/atualizar Channels do tenant (declarativo). | Publicar no Admin VendaX cria/atualiza a instância no Linktor; `settings.vendorId` fica associado; credenciais/pairing intactos. |
| **L2 · Identidade & robustez** | `vendorId` por canal, `customerId`↔contato (telefone/auto-link), correlação de conversa no outbound, idempotência bidirecional, mapeamento `channel_type`. | Conversa correta é resolvida nas duas pontas sem vazamento entre tenants; dedup comprovado; tipos de canal mapeados. |
| **L3 · Rich objects & tipos** | `messageType` além de texto: áudio (→ transcrição no Core), cotação e demais rich objects ADR-010 no outbound; anexos. | Rich object da IA (ex.: cotação) entregue e renderizado; áudio do cliente vira transcrição no Core. |
