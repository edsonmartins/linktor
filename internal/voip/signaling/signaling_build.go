...
import (
	...
	"github.com/msgfy/linktor/internal/voip/wanode"
...
var (
	// Audio calls still need the stable WaCalls/main capability bytes. The
	// feat/video-calls branch changed the offer capability for video; using the
	// video capability for normal audio calls makes some WhatsApp clients send a
	// terminate immediately, which surfaces in the UI as "chamada encerrada antes
	// de iniciar".
	capabilityAudioOffer     = []byte{0x01, 0x05, 0xf7, 0x09, 0xe4, 0xbb, 0x07}
	capabilityVideoOffer     = []byte{0x01, 0x05, 0xf7, 0x09, 0xe4, 0xbb, 0x13}
	capabilityAudioPreaccept = []byte{0x01, 0x05, 0xff, 0x09, 0xe4, 0xbb, 0x07}
	capabilityVideoPreaccept = []byte{0x01, 0x05, 0xf7, 0x09, 0xe4, 0xbb, 0x07}
)
...
func BuildOfferStanza(ctx context.Context, sock core.VoipSocket, callID string, callKey []byte, peerJid types.JID, isVideo bool) (waBinary.Node, error) {
	...
	if !hasPrivacy {
		// Explicit log so operators can correlate immediate reason=busy responses
		// with the missing privacy token (WhatsApp rejects calls from unknown
		// contacts that never exchanged messages with us).
		slogWarnNoPrivacy(ctx, callID, peerJid)
	}
	...
	capability := capabilityAudioOffer
	if isVideo {
...
func BuildPreacceptStanza(peerJid types.JID, callID string, callCreator types.JID, isVideo bool) waBinary.Node {
	capability := capabilityAudioPreaccept
	if isVideo {
	...
	return waBinary.Node{
		...
		Content: []waBinary.Node{{
			...
			Content: []waBinary.Node{
				{Tag: "audio", Attrs: waBinary.Attrs{"enc": "opus", "rate": "16000"}},
				{Tag: "encopt", Attrs: waBinary.Attrs{"keygen": "2"}},
				{Tag: "capability", Attrs: waBinary.Attrs{"ver": "1"}, Content: capability},
			},
		}},
	}
}
...
// slogWarnNoPrivacy emits a single explicit warning when an outgoing offer is
// built without a privacy token. WhatsApp typically responds with reason=busy
// (before the destination phone even rings) in this case, so surfacing it in
// the logs lets operators correlate the failure with the actual cause instead
// of investigating relay/ICE which are working fine.
func slogWarnNoPrivacy(_ context.Context, callID string, peerJid types.JID) {
	slog.Warn("offer built without privacy token; peer may reject with reason=busy",
		"call_id", callID, "peer", peerJid.String(),
		"hint", "send a WhatsApp message to this contact first so it issues a privacy token")
}
